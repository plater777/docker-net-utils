...
RUN apt-get update && \
apt-get install -y apt-transport-https arping bro bro-aux broctl dhcp-probe dnsrecon dsniff ethtool flow-tools flowscan ftp \
geoip-bin guessnet inetutils-ping inetutils-tools ipcalc iproute2  masscan msktutil nbtscan ndiff net-tools netcat netdiscover \ 
